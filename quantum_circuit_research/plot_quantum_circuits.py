...
json_path = os.path.join(current_dir, "results", "quantum_scaling_results.json")
output_path = os.path.join(current_dir, "results", "quantum_cost_progression_v2.png")

def main():
    if not os.path.exists(json_path):
        print(f"Error: JSON data file not found at '{json_path}'. Run sweep orchestrator first.")
        return

    with open(json_path, "r") as f:
        results = json.load(f)

    # 2x7 grid: Row 1 = Cumulative Time (%), Row 2 = Absolute Time (s, log scale)
    fig, axes = plt.subplots(2, 7, figsize=(28, 9), dpi=300)
    
    topologies = list(results.keys())
    
    # 7-color sequential palette (Yellow-Green-Blue)
    size_colors = [
        "#c7e9b4",  # Size 1 (lightest)
        "#7fcdbb",  # Size 2
        "#41b6c4",  # Size 3
        "#1d91c0",  # Size 4
        "#225ea8",  # Size 5
        "#253494",  # Size 6
        "#081d58"   # Size 7 (darkest)
    ]
    ...
    for col_idx, topo_name in enumerate(topologies):
        ...
        for size_idx, config in enumerate(size_configs):
            ...
            ax_cum.plot(x_steps, seq_cum_time_pct, label=f"Seq: {size_label}", color=color, linestyle="-", linewidth=2.0)
            ax_cum.plot(x_steps, sliced_cum_time_pct, label=f"Par: {size_label}", color=color, linestyle="--", linewidth=2.0)
            
            # --- Plot Row 2: Absolute seconds (log scale) ---
            ax_abs.plot(x_steps[1:], seq_cum_time_sec[1:], label=f"Seq: {size_label}", color=color, linestyle="-", linewidth=2.0)
            ax_abs.plot(x_steps[1:], sliced_cum_time_sec[1:], label=f"Par: {size_label}", color=color, linestyle="--", linewidth=2.0)
    ...
    fig.suptitle("Quantum Circuit Tensor Network Contraction: Sequential vs. Parallel Sliced (7x7 Sweep)", fontweight="bold", y=0.98, fontsize=18)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(output_path, dpi=300)
    print(f"Successfully generated 2x7 quantum step progression plot at: {output_path}")
