...
function tensordot_adaptive(A::AbstractArray{Float64}, B::AbstractArray{Float64}, inds_A::Vector{String}, inds_B::Vector{String}, inds_contract::Vector{String}, num_threads::Int, is_heavy::Bool)
...
# Combined hybrid recursive contractor
function contract_hybrid_node(node_id::Int, num_initial::Int, parent_to_step::Dict{Int, ContractionStep},
                              node_edges::Vector{Vector{String}}, initial_tensors::Vector{Array{Float64}},
                              num_threads::Int, heavy_threshold_size::Int)
    ...
    out_edge_count = length(node_edges[node_id])
    # Est size: 2^out_edge_count
    est_size = 2^out_edge_count
    is_heavy = (est_size >= heavy_threshold_size)
    
    if is_heavy
        # Heavy node: evaluate left and right (sequentially to avoid memory blowup and task thrashing),
        # then parallelize the heavy contraction across all BLAS threads
        res_L = contract_hybrid_node(left_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
        res_R = contract_hybrid_node(right_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
        return tensordot_adaptive(res_L, res_R, node_edges[left_id], node_edges[right_id], step.legs, num_threads, true)
    else
        # Small / medium subtree: fork independent branch tasks to maximize tree-level concurrency
        if (left_id > num_initial || right_id > num_initial)
            task_L = Threads.@spawn contract_hybrid_node(left_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
            res_R = contract_hybrid_node(right_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
            res_L = fetch(task_L)
            return tensordot_adaptive(res_L, res_R, node_edges[left_id], node_edges[right_id], step.legs, num_threads, false)
        else
            res_L = contract_hybrid_node(left_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
            res_R = contract_hybrid_node(right_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold_size)
            return tensordot_adaptive(res_L, res_R, node_edges[left_id], node_edges[right_id], step.legs, num_threads, false)
...
function contract_combined_hybrid(job_dir::String, num_threads::Int)
    ...
    heavy_threshold = 10_000 # >= 10^4 elements switches to multithreaded GEMM
    
    result_tensor = contract_hybrid_node(root_id, num_initial, parent_to_step, node_edges, initial_tensors, num_threads, heavy_threshold)
    return result_tensor[1]
end

if length(ARGS) < 1
    println("Usage: julia hybrid_combined_contractor.jl <job_dir> [num_threads]")
    exit(1)
end
...
# Warmup pass
val_warm = contract_combined_hybrid(job_dir, num_threads)

# Profiled pass
t0 = time_ns()
val = contract_combined_hybrid(job_dir, num_threads)
t1 = time_ns()
